...
import { generateDailyData, generateForecastData, getGradeColor, getGradeLabel } from "@/data/mockData";
...
export default function ComplianceDetailPage() {
  ...
  const { companies, blockchainRecords } = useSocketSimulator();
  const company = companies.find((c) => c.id === Number(id));

  if (!company) {
    return (
      <div className="text-center py-16">
        <p className="text-xl font-bold text-foreground">Company not found</p>
        <div className="mt-4 text-primary cursor-pointer hover:underline" onClick={() => navigate("/dashboard/profiles")}>← Back to Profiles</div>
      </div>
    );
  }

  const dailyData = generateDailyData(company.emissions, company.trend);
  const forecastData = generateForecastData(company.emissions, company.trend);
  ...
  const companyRecords = blockchainRecords.filter((r) => r.company === company.name).slice(0, 5);

  return (
    <div className="space-y-6 animate-fade-up">
      <div className="text-sm text-primary cursor-pointer hover:underline" onClick={() => navigate("/dashboard/profiles")}>← All Companies</div>
      ...
      {companyRecords.length > 0 && (
        <div className="card-dashboard p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-primary cursor-pointer hover:underline">View on Polygonscan ↗</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-dash-border text-muted-foreground">
                <th className="text-left py-2">Hash</th><th className="text-left py-2">Timestamp</th><th className="text-left py-2">IPFS CID</th><th className="text-left py-2">Status</th>
              </tr></thead>
              <tbody>
