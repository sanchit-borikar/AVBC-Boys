...
import { getGradeColor, getGradeFromScore } from "@/data/mockData";
...
/* ─── per-company heatmap config ─── */
interface GlowConfig {
  outerRadius: number; innerRadius: number;
  color: string; outerOpacity: number; innerOpacity: number;
}
const GLOW_MAP: Record<string, GlowConfig> = {
  "CarbonHeavy Mfg":      { outerRadius: 80, innerRadius: 50, color: "#dc2626", outerOpacity: 0.15, innerOpacity: 0.25 },
  "SteelCore Industries":  { outerRadius: 60, innerRadius: 40, color: "#dc2626", outerOpacity: 0.15, innerOpacity: 0.25 },
  "ChemWorks Inc":         { outerRadius: 55, innerRadius: 35, color: "#ea580c", outerOpacity: 0.15, innerOpacity: 0.25 },
  "PowerGrid East":        { outerRadius: 50, innerRadius: 30, color: "#d97706", outerOpacity: 0.12, innerOpacity: 0.2 },
  "MetroTransit":          { outerRadius: 40, innerRadius: 25, color: "#d97706", outerOpacity: 0.12, innerOpacity: 0.2 },
  "EcoFreight Ltd":        { outerRadius: 30, innerRadius: 20, color: "#0891b2", outerOpacity: 0.1,  innerOpacity: 0.15 },
  "SolarGen Corp":         { outerRadius: 20, innerRadius: 12, color: "#059669", outerOpacity: 0.1,  innerOpacity: 0.15 },
  "GreenMove Transport":   { outerRadius: 18, innerRadius: 10, color: "#059669", outerOpacity: 0.1,  innerOpacity: 0.15 },
};
...
export default function LiveMapPage() {
  const { companies, alerts } = useSocketSimulator();
  ...
  if (dataFilter === "verified") filtered = filtered.filter((c) => c.status === "verified");
  else if (dataFilter === "self-reported") filtered = filtered.filter((c) => c.status === "flagged");
  ...
  const flaggedCompanies = companies.filter((c) => c.status === "flagged");
  ...
  const sectorEmoji = (sector: string) => sector === "Industry" ? "🏭" : sector === "Transport" ? "🚗" : "⚡";
    ...
    <div className="flex flex-col lg:flex-row gap-0 -m-4 lg:-m-6 h-[calc(100vh-136px)]">
                          ...
                          <div style={{ padding: 16 }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
                              <div style={{ display: "flex", justifyContent: "space-between" }}>
                                <span style={{ color: "#64748b" }}>📡 IoT Reading</span>
                                <span style={{ fontFamily: "JetBrains Mono, monospace", fontWeight: 500 }}>{c.iot.toLocaleString()} kg CO₂e</span>
                              </div>
                              <div style={{ display: "flex", justifyContent: "space-between" }}>
                                <span style={{ color: "#64748b" }}>🛰️ NASA GEOS-CF</span>
                                <span style={{ fontFamily: "JetBrains Mono, monospace", fontWeight: 500 }}>{c.satellite.toLocaleString()} kg CO₂e</span>
                              </div>
                              <div style={{ display: "flex", justifyContent: "space-between" }}>
                                <span style={{ color: "#64748b" }}>📊 Discrepancy</span>
                                <span style={{
                                  fontFamily: "JetBrains Mono, monospace",
                                  fontWeight: 700,
                                  color: discColor(c.discrepancy),
                                }}>{c.discrepancy}%</span>
                              </div>
                            </div>
      ...
      <div className="hidden lg:block w-80 bg-card border-l border-dash-border overflow-y-auto p-4 space-y-5">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {[
            { value: "847", label: "Companies", color: "#2563eb" },
            { value: String(flaggedCompanies.length), label: "Flagged", color: "#dc2626" },
            { value: `${((verifiedCount / companies.length) * 100).toFixed(1)}%`, label: "Verified", color: "#059669" },
            { value: "3,241", label: "On-Chain", color: "#7c3aed" },
          ].map((s) => (
