import { generateHourlyData, gradeDistribution, sectorEmissionMonthly } from "@/data/mockData";
...
export default function AnalyticsPage() {
  ...
  const topImprovers = [...companies].filter((c) => c.trend === "down").sort((a, b) => b.trendPct - a.trendPct).slice(0, 5);
  const mostFlagged = [...companies].filter((c) => c.flagged).slice(0, 5);

  const sectorEmissions = [
    { name: "Industry", value: companies.filter((c) => c.sector === "Industry").reduce((s, c) => s + c.emissions, 0), color: "#2563eb" },
    { name: "Transport", value: companies.filter((c) => c.sector === "Transport").reduce((s, c) => s + c.emissions, 0), color: "#0891b2" },
    { name: "Energy", value: companies.filter((c) => c.sector === "Energy").reduce((s, c) => s + c.emissions, 0), color: "#7c3aed" },
  ];
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={sectorEmissions} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} label={({ name }) => name}>
                {sectorEmissions.map((e, i) => <Cell key={i} fill={e.color} />)}
              </Pie>
              <Tooltip 
                contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", padding: "10px 14px", fontFamily: "'Plus Jakarta Sans', sans-serif" }} 
                itemStyle={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }} 
